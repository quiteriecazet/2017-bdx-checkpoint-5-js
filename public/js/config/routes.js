...
export default ['$stateProvider', '$urlRouterProvider', '$locationProvider', ($stateProvider, $urlRouterProvider, $locationProvider) => {
    ...
    */
    ...
    $stateProvider.state('algo2', {
        url: '/algo2',
        controller: ['UsersService', '$stateParams', '$state', '$scope', function (UsersService, $stateParams, $state, $scope) {
            $scope.distance = 14928418679754190000;
            $scope.thickness = 0.0001;
            $scope.count = 0;

            while ($scope.thickness < $scope.distance) {
                console.log($scope.thickness)
                $scope.thickness += $scope.thickness;
                $scope.count++;

            }
        }],
        template: '<navbar /><div class="container"><p>For a distance of {{distance}}m, I would have to fold my paper {{count}} times, you can view every step on the console.<ui-view></ui-view></div>'
    })
}]